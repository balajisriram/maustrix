...
%mcc -m -I C:\psychtoolbox\Psychtoolbox -I \\Reinagel-lab.ad.ucsd.edu\rlab\Rodent-Data\BCoreAdmin\ -d standalone BCoreServer.m
%but this yields:
%
...
% The file
%    '\\Reinagel-lab.ad.ucsd.edu\rlab\Rodent-Data\BCoreAdmin\getBCoreHeatInfo.m'
%
%    is not in the application's expanded CTF archive at
%     'C:\Documents and Settings\rlab\Desktop\BCore\bootstrap\standalone\BCoreSe
% rver_mcr'.
...
% cannot be executed.
%
% Error in ==> BCoreServer at 48
%
% MATLAB:err_parse_cannot_run_m_file

function BCoreServer
setupEnvironment;
addJavaComponents();
...
% fHeight=fHeight*2;
f = figure('Visible','off','MenuBar','none','Name',sprintf('BCore control: %s IP: %s',server_name, myIP),'NumberTitle','off','Resize','off','CloseRequestFcn',@cleanup,'Units','pixels','Position',[50 50 fWidth fHeight]);
...
%             size(all_other_subjects)
            ...
            y=errordlg(['call erik or philip before closing this box.  quitting due to error: ' ex.message],'BCore error','modal')
            uiwait(y)
            cleanup
        end
    end
...
rx=BCore(fullfile(dataPath, 'ServerData'),0); %load from file

[rx ids]=emptyAllBoxes(rx,'BCoreServer starting','BCore');
if length(ids)>0
    ids
    warning('found subjects in boxes at BCoreServer startup')
end
end

function [r rx sys]=stopServer(r,rx,servePump,sys,er,subjects)
fprintf('%s: quitting BCore server\n',datestr(now))
    ...
    for i=1:length(connectedClients) %this needs to happen after disconnections have been removed to prevent fast reconnects from confusing me!
        ...
        if ~clientIsRegistered(r,connectedClients{i}) || reregistered
            ...
            if quit
                ...
                %                 [quit com]=sendToClient(r,connectedClients{i},constants.priorities.IMMEDIATE_PRIORITY,constants.serverToStationCommands.S_UPDATE_SOFTWARE_CMD,{'svn://132.239.158.177/projects/BCore/trunk'});
                %                 if ~quit
                %                     timeout=10.0;
                ...
                if ~isempty(z)
                    ...
                    [rx quit]=updateBCoreFromClientBCore(r,rx,connectedClients{i});

                    if ~quit && tf
                        nonPersistedBCore=makeNonpersistedBCoreForStationMAC(rx,mac);
                        if ~isempty(nonPersistedBCore)
                            gotGoodBCore=true;
                            [quit com]=sendToClient(r,connectedClients{i},constants.priorities.IMMEDIATE_PRIORITY,constants.serverToStationCommands.S_START_TRIALS_CMD,{nonPersistedBCore});
                            if ~quit
                                nonPersistedBCore=[];
                                fprintf('\nsent BCore to %s\n',mac)
                                timeout=5.0;
                                quit=waitForAck(r,com,timeout,'waiting for ack from sent BCore');
                            end
...
%                 if isempty(z) || ~tf || quit || ~gotGoodBCore
                if isempty(z) || quit || (~gotGoodBCore && tf)  % 9/22/08 - don't toss clients w/o a subject running
                    z
                    quit
                    gotGoodBCore
                    tf                    
                    
                    % toss client if there is no zone, or if quit is received, or if client is registered but did not get a good BCore
...
[rx ids]=emptyAllBoxes(rx,'BCoreServer cleanup','BCore');
if length(ids)>0
    ids
    warning('subjects were left in boxes at BCoreServer cleanup')
end
