...
fWidth=1000;
fHeight=900;
f = figure('Visible','off','MenuBar','none','Name','BCore control','NumberTitle','off','Resize','off','CloseRequestFcn',@cleanup,'Units','pixels','Position',[50 50 fWidth fHeight]);
...
heats={'heat 1','heat 2','overnight'}; %need to get from BCore (which you need to get from server)
heatTStr='heat:';
...
heatB=uicontrol(heatP,'Style','togglebutton','String','run trials','Units','pixels','Position',[4*margin+shortTWidth+ddWidth+longTWidth margin bWidth oneRowHeight],'Callback',{@heatB_Callback}); %need to get initial state from BCore/server
...
stations=[1 1 1;1 1 2;1 1 3;1 2 1;1 2 2;1 2 3;2 1 1;2 1 2;2 2 1;2 2 2;2 2 3;2 3 1;2 3 3]; %need to get from BCore (which you need to get from server)
...
% -current BCore (contains info about stations, subjects, heats, and assignedTrialManagers)
