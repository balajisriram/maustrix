classdef flatHazard<delayManager
    ...
    methods
        function f=flatHazard(percentile, value, fixedDelayMs)
            ...
            assert(isscalar(percentile) && isnumeric(percentile) && percentile>0 && percentile<=1,'flatHarard:inproperValue','percentile must be >0 and <=1')
            f.percentile=percentile;
            
            assert(isscalar(value) && isnumeric(value) && value>0,'flatHarard:inproperValue','value must be >0');
            f.value=value;
            
            assert(isscalar(fixedDelayMs) && isnumeric(fixedDelayMs) && fixedDelayMs>=0,'flatHarard:inproperValue','fixedDelayMs must be >=0');
