classdef nTrialsThenWait<scheduler
    ...
    methods
        function s=nTrialsThenWait(vpossibleNumTrials,probOfNumTrials,possibleHoursBetweenSessions,probOfHoursBetweenSessionsarargin)
            ...
            if all(vpossibleNumTrials>=0) && all(possibleHoursBetweenSessions>=0)
                %these are a list off possible values
                s.possibleNumTrials=vpossibleNumTrials;
                s.possibleHoursBetweenSessions=possibleHoursBetweenSessions;
            else
                error('all values of possibleNumTrials and possibleHoursBetweenSessions must be >= 0')
            end

            if (sum(probOfNumTrials)==1) && (sum(probOfHoursBetweenSessionsarargin)==1) && length(vpossibleNumTrials)==length(probOfNumTrials) && length(possibleHoursBetweenSessions)==length(probOfHoursBetweenSessionsarargin)
                %these are the probability of each of the possible values
                s.probOfNumTrials=probOfNumTrials;
                s.probOfHoursBetweenSessions=probOfHoursBetweenSessionsarargin;
            else
                vpossibleNumTrials;
                probOfNumTrials;
                possibleHoursBetweenSessions;
                probOfHoursBetweenSessionsarargin;
