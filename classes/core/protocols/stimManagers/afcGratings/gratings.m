classdef gratings<stimManager
    % gratings
    % This class is manages grating presentation. It manages drawing logic
    % of individual gratings moving or static. Can be called by
    % afcGratings, etc. etc.
    ...
    methods
        function s=gratings(pixPerCycs,driftfrequencies,orientations,phases,contrasts,maxDuration,radii,radiusType, annuli,location,...
            ...
            assert(islogical(doCombos),...
                'afcGratings:afcGratings:invalidInput','doCombos not in the right format');
            s.doCombos = doCombos;
            
            % pixPerCycs
            assert(isnumeric(pixPerCycs) && all(pixPerCycs>0),'afcGratings:afcGratings:invalidInput','pixPerCycs not in the right format');
            L = length(pixPerCycs);
            ...
            assert(doCombos || length(driftfrequencies)==L ,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            ...
            assert(doCombos || length(orientations)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            ...
            assert(doCombos || length(phases)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            s.phases = phases;
            ...
            assert(doCombos || length(contrasts)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            s.contrasts = contrasts;
                        
            % maxDuration
            assert(isnumeric(maxDuration) && all(maxDuration>0),...
                'afcGratings:afcGratings:invalidInput','maxDuration not in the right format');
            assert(doCombos || length(maxDuration)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            ...
            assert(doCombos || length(radii)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            s.radii = radii;
            ...
            assert(doCombos || length(annuli)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            s.annuli = annuli;
            ...
            assert(doCombos || length(location)==L,'afcGratings:afcGratings:incompatibleValues','the lengths don''t match.');
            s.location = location;
