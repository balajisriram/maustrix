classdef bipartiteField<stimManager
    ...
    methods
        function s=bipartiteField(receptiveFieldLocation,frequencies,duration,repetitions,maxWidth,maxHeight,scaleFactor,interTrialLuminance)
            ...
            if (isvector(receptiveFieldLocation) && length(receptiveFieldLocation) == 2) || isa(receptiveFieldLocation,'RFestimator')
                s.receptiveFieldLocation = receptiveFieldLocation;
            else
                error('receptiveFieldLocation must be a two-element array [xPos yPos] as fractional locations, or an RFestimator');
            end
            % frequencies
            if isvector(frequencies) && isnumeric(frequencies)
                if all(frequencies>=2)
            ...
            % repetitions
            if isscalar(repetitions) && isnumeric(repetitions)
