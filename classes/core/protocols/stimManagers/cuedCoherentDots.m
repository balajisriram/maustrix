classdef cuedCoherentDots<stimManager
    ...
    methods
        function s=cuedCoherentDots(screen_width,screen_height,num_dots,coherence,speed,contrast, ...
            ...
            rewards_and_probability = rewards_and_probability;
            ...
            % cue_sounds {10}
            if iscell(cue_sounds)
                if all(cellfun(@ischar, cue_sounds(1,:))) && all(size(cue_sounds)==size(rewards_and_probability)) && all(cellfun(@isnumeric,cue_sounds(2,:)))
                    s.cue_sounds = cue_sounds;
                else
