classdef filteredNoise<stimManager
    ...
    methods
        function s=filteredNoise(in,maxWidth, maxHeight, scaleFactor, interTrialLuminance)
            ...
            if isstruct(in)  &&   all(ismember(fieldNames,fields(in)))  % create object using specified values
                ...
                for j=1:length(in)
                    ...
                    if (isscalar(in.startFrame) && isinteger(in.startFrame) && in.startFrame>0)
                        in(j).startFrame=uint32(in.startFrame); %otherwise our frame indices can't exceed the max of the datatype of the startframe, and there's no colon operator on uint64...
