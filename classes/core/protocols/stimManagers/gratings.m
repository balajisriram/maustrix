    ...
    methods
        function s=gratings(pixPerCycs,driftfrequencies,orientations,phases,contrasts,durations,radii,annuli,location, ...
            ...
            elseif iscell(doCombos) && (length(doCombos)==3)
                s.doCombos = doCombos{1}; if ~islogical(doCombos{1}), error('doCombos should be a logical'), end;
                s.ordering.method = doCombos{2}; if ~ismember(doCombos{2},{'twister','state','seed'}), error('unknown ordering method'), end;
                s.ordering.seed = doCombos{3}; if (~(ischar(doCombos{3})&&strcmp(doCombos{3},'clock'))&&(~isnumeric(doCombos{3}))), ...
                        error('seed should either be a number or set to ''clock'''), end;
                s.ordering.includeBlank = false;
            elseif iscell(doCombos) && (length(doCombos)==4)
                s.doCombos = doCombos{1}; if ~islogical(doCombos{1}), error('doCombos should be a logical'), end;
                s.ordering.method = doCombos{2}; if ~ismember(doCombos{2},{'twister','state','seed'}), error('unknown ordering method'), end;
                s.ordering.seed = doCombos{3}; if (~(ischar(doCombos{3})&&strcmp(doCombos{3},'clock'))&&(~isnumeric(doCombos{3}))), ...
                        error('seed should either be a number or set to ''clock'''), end;
                s.ordering.includeBlank = doCombos{4}; if ~islogical(doCombos{4}), error('includeBlank should be a logical'), end;
            else
            ...
            elseif isa(pixPerCycs,'SFestimator');
                s.pixPerCycs = pixPerCycs;
            else
                error('pixPerCycs must be numbers OR an SFEstimator Object');
            end
            % driftfrequency
            if isvector(driftfrequencies) && isnumeric(driftfrequencies) && all(driftfrequencies)>0
            ...
            % contrasts
            if isvector(contrasts) && isnumeric(contrasts) && all(contrasts>=0 & contrasts<=1)
            ...
            % radii
            if isnumeric(radii) && all(radii>0) && all(~isinf(radii))
                s.radii=radii;
            elseif iscell(radii) && (length(radii)==2)
                s.radii = radii{1};
                s.radiusType = radii{2};
                if ~all(radii{1}>0) || ~any(strcmp(radii{2},{'gaussian','hardEdge'}))
            ...
            elseif isa(location,'RFestimator')
                s.location=location;
            elseif isa(location,'wnEstimator')
                s.location=location;
            else
