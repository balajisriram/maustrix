classdef stimSpec
    ...
    properties
        ...
        stimulus = zeros(1,1,1);
        transitions = {[], 0};
        stimType = 'loop';
        startFrame = 1;
        framesUntilTransition = [];
        autoTrigger = []; % for now the "distribution" is a unit random criterion between 0 and 1
        scaleFactor=0;
        isFinalPhase = 0;
        hz=0;
        phaseType=[];
    ...
    methods
        ...
        switch nargin
            ...
            case {12 13 14 15}
                ...
                if ~isempty(varargin{10}) && ischar(varargin{10}) && ismember(varargin{10},{'reinforced','pre-request','pre-response',...
                        'discrim','post-discrim','itl','earlyPenalty'})
                ...
                % punishResponses
                if islogical(varargin{12}) && length(varargin{12})==1
                    spec.punishResponses=varargin{12};
                else
                    error('punishResponses must be a boolean');
                end

                % isStim and indexPulses (optional)
                if nargin>=13 && ~isempty(varargin{13})
                    spec.isStim=varargin{13};
                end
                ...
                if nargin==15 && ~isempty(varargin{15})
                    spec.ledON=varargin{15};
                else
                    spec.ledON=false;
                end
                if isvector(spec.ledON) && islogical(spec.ledON) && length(spec.ledON)<=2
                    %pass
                else
        ...
        function out = getAutoTrigger(spec)
            out=spec.autoTrigger;
        end
        
        function fr=getFramesUntilTransition(spec)
            % stimSpec.framesUntilTransition accessor function
            % returns the number of frames until this phase transitions associated with the given stimSpec object
