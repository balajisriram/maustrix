classdef trainingStep
    ...
    methods
        function t=trainingStep(trialManager,stimManager,criterion,scheduler,svnRevision,svnCheckMode,stepName)
            ...
            if isa(trialManager,'trialManager') && isa(stimManager,'stimManager') && isa(criterion,'criterion') && isa(scheduler,'scheduler')
                t.trialManager = trialManager;
                t.stimManager = stimManager;
                t.criterion = criterion;
                t.scheduler = scheduler;

                if ischar(svnCheckMode) && (strcmp(svnCheckMode,'session') || strcmp(svnCheckMode,'trial') || strcmp(svnCheckMode,'none'))
                    t.svnCheckMode = svnCheckMode;
                else
                ...
                if ~strcmp(t.svnCheckMode,'none')
                    try
                        [t.svnRevURL t.svnRevNum]=checkTargetRevision(svnRevision);
                    catch ex
                ...
                isa(trialManager,'trialManager')
                isa(stimManager,'stimManager')
                isa(criterion,'criterion')
                isa(scheduler,'scheduler')
