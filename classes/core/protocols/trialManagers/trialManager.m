classdef trialManager
    ...
    methods
        function t=trialManager(sndMgr,reinfMgr,delMgr,frameDropCorner,dropFrames,requestPorts,saveDetailedFramedrops,customDescription,responseWindowMs,showText)
            ...
            assert(all(ismember(trialManager.requiredSoundNames, getSoundNames(sndMgr))),'trialManager:incompleteInputs','provide a soundManager with all required sounds');
            t.soundMgr=sndMgr;
            ...
            % requestPorts
            assert(ismember(requestPorts,{'none','all','center'}),'trialManager:incorrectInput','requestPort should be ''none'',''all'' or ''center''');
            ...
            % showText
            assert(ismember(showText,{'full','light','off'}),'trialManager:incorrectInput','only allowed values for showText is ''full'',''light'',''off''');
            t.showText=showText;
        end
        ...
        function [tm, updateTM, newSM, updateSM, stopEarly, tR, st, updateRM] = doTrial(tm,st,sm,sub,r,rn,tR,sessNo,cR)
            ...
            assert(stationOKForTrialManager(tm,st),'trialManager:doTrial:incompatibleValues','station not okay for trial manager');
            ...
            tR(tRInd).box = structize(getBoxFromID(r,getBoxIDForSubjectID(r,getID(sub))));
            ...
            stns=getStationsForBoxID(r,getBoxIDForSubjectID(r,getID(sub)));
            for stNum=1:length(stns)
                tR(tRInd).stationIDsInBox{stNum} = stns(stNum).id;
            end
            
            tR(tRInd).subjectsInBox = getSubjectIDsForBoxID(r,getBoxIDForSubjectID(r,getID(sub)));
            ...
            [tempSoundMgr, updateSndM] = cacheSounds(getSoundManager(tm),st);
            ...
            drawnow;
            currentValveStates=verifyValvesClosed(st); % #### do we need this?
            
            pStr=[tR(tRInd).protocolName '(' num2str(tR(tRInd).protocolVersion.manualVersion) 'm:' num2str(tR(tRInd).protocolVersion.autoVersion) 'a)' ' step:' num2str(tR(tRInd).trainingStepNum) '/' num2str(tR(tRInd).numStepsInProtocol) ];
            ...
            if ~isempty(st.datanet)
                ...
                if ~isempty(getDatanet(st))
                    [garbage, stopEarly] = handleCommands(getDatanet(st),[]);
                end
                if ~stopEarly
                    commands=[];
                    ...
                    [gotAck] = sendCommandAndWaitForAck(getDatanet(st), commands);
            ...
            if ~isempty(st.datanet) %&& ~stopEarly
                [garbage, garbage] = handleCommands(st.datanet,[]);
                ...
                cparams.time = now;
                commands.arg=cparams;
                [gotAck] = sendCommandAndWaitForAck(st.datanet, commands);
            end
        ...
        function [tm,trialDetails, result, spec, rewardSizeULorMS, requestRewardSizeULorMS, ...
                ...
                updateTrialState(tm, ~, result, spec, ports, lastPorts,~,requestPorts,lastRequestPorts,~,trialRecords, ~, ~, ~, ...
                floatprecision, textures, destRect,requestRewardDone, ~,compiledRecords, subject)
    ...
    methods (Access=private)
        ...
        function [tm, Quit, trialRecords, eyeData, eyeDataFrameInds, gaze, frameDropCorner, station, updateRM] ...
                ...
                station, manual,timingCheckPct,textLabel,rn,subID,stimID,protocolStr,ptbVersion,ratrixVersion,trialLabel,...
                originalPriority, eyeTracker, frameDropCorner,trialRecords,compiledRecords,subject)
            ...
            while ~done && ~Quit;
                ...
                if ~isempty(rn) || strcmp(station.rewardMethod,'serverPump')
                    [done, Quit, phaseRecords(thisRewardPhaseNum).valveErrorDetails, serverValveStates, serverValveChange, ...
                        ...
                        = tm.handleServerCommands(rn, done, Quit, requestRewardStarted, ...
        ...
        function [tm, quit, trialRecords, eyeData, eyeDataFrameInds, gaze, station, updateRM] ...
            ...
            try
                ...
                for i=1:length(stimSpecs)
                    ...
                    if window>0
                        phaseData{i}.destRect = determineDestRect(tm, window, metaPixelSize, stim, phaseData{i}.strategy);
                ...
                [tm, quit, trialRecords, eyeData, eyeDataFrameInds, gaze, ~, station, updateRM] ...
                    ...
                    targetOptions, distractorOptions, requestOptions, interTrialLuminance, interTrialPrecision, ...
                    station, manual,timingCheckPct,textLabel,rn,subID,stimID,protocolStr,ptbVersion,ratrixVersion,trialLabel,...
                    originalPriority,eyeTracker,frDropCorner,trialRecords,compiledRecords,subject);
    ...
    methods (Static)
        ...
        function destRect = determineDestRect(window, metaPixelSize, stim, strategy)
            ...
            assert(any(scaleFactor.*[stimheight stimwidth]<=[scrHeight scrWidth]),'trialManager:determineDestRect:incorrectValue','metaPixelSize is too large');
        ...
        function testStimOGL() %laboratory for finding and fixing framedrops - call like testStimOGL(trialManager)
            ...
            try
                ...
                [tm.soundMgr, garbage]=cacheSounds(tm.soundMgr,st);
