classdef ratrix
    %%% RATRIX
    % ## should be split into standAloneRatrix in the future
    properties
        serverDataPath
        dbpath
        subjects
    ...
    methods
        function r = ratrix(dataPath,replaceExistingDB,standAlonePath)
            switch nargin
                case 2
                ...
                otherwise
                    error('ratrix:ratrix:incorrectArgumentNumber','unsupported number of arguments (=%d)',nargin);
            end
        end
        ...
        function r = set.standAlonePath(r,path)
            assert(isdir(path),'ratrix:set.standAlonePath:incorrectValue','Argument to permanentStorePath (%s) is not a directory',path)
        ...
        function r = addBox(r,b)
            ...
            assert(~any(ismember(b.id,getBoxIDs(r))),'ratrix:addBox:incompatibleValue','ratrix already contains a box with that id');
        ...
        function r = addStationToBoxID(r,s,bID)
            ...
            assert(~isempty(bx) && checkPath(s.path),'ratrix:addStationToBoxID:incorrectValue','box id not in ratrix or couldn''t get to station path');
            assert(isempty(getStationByID(r,s.id)),'ratrix:addStationToBoxID:incompatibleValue','station id already exists in ratrix');
        ...
        function r=commandAllBoxes(r,cmd,comment,auth,secsAcknowledgeWait)
            assert(ismember(cmd,{'start','stop'}),'ratrix:commandBoxIDStationIDs:incorrectValue','cmd must be start or stop');
            assert(r.authorCheck(auth),'ratrix:commandBoxIDStationIDs:incorrectValue','author chouls be authorized');
        ...
        function r = establishDB(r,dataPath,replaceExistingDB)
            ...
            assert(~isempty(pathstr) && isempty(name),'ratrix:estabilshDB:incompatibleParamValue','must provide a fully resolved path to a new or existing data base directory');
            ...
            elseif length(found)==1
                if replaceExistingDB
                    ...
                    fprintf('done loading ratrix db: %g s elapsed',GetSecs()-startTime);
                    
                    r=saved.r;
                end
                
            else
                error('ratrix:estabilshDB:unknownError','found got %d matches for dbpath',length(found));
            end
        ...
        function [compiledRecords]=getCompiledRecordsForSubjectID(r,sID)
            fprintf('loading compiled records for %s (from ratrix)\n',sID);
        ...
        function out = getProtocolForSubjectID(r,subID)
            ...
            assert(index>0,'ratrix:getProtocolForSubjectID:incorrectParams','no such subject id');
        ...
        function [trialRecords, localIndex, sessionNumber, compiledRecords]=getTrialRecordsForSubjectID(r,sID,filter, trustOsRecordFiles)
            ...
            %
            fprintf('loading records for %s (from ratrix)\n',sID);
            startTime=GetSecs();
        ...
        function out=makeNonpersistedRatrixForStationMAC(r,m)
            if isMACaddress(m)
                ...
                out=makeNonpersistedRatrixForStationID(r,st.id);
            else
        ...
        function r = mergeMiniIntoRatrix(r,newR)
            ...
            for i=1:length(newSubjectIDs)
                ...
                if member
                    ...
                    warning('This subject id (%s) is not in the current server side ratrix, but is in the client side ratrix',newSubjectIDs{i});
                end
            end
        end
        ...
        function updateTrialRecordsForSubjectID(r,sID,trialRecords)
            fprintf('saving records for %s (from ratrix)\n',sID);
            startTime=GetSecs();
            bID=getBoxIDForSubjectID(r,sID);
            if bID==0
                subPath=getServerDataPathForSubjectID(r,sID);
    ...
    methods (Access = protected)
        ...
        function saveDB(r,alsoReplaceSubjectData)
            ...
            if ~isempty(pathstr) && ~isempty(name)
                ...
                error('ratrix:improperDataType','can''t read database path %s',r.dpbpath);
            end
        ...
        function r=commandBoxIDStationIDs(r,cmd,boxID,stationIDs,comment,auth,secsAcknowledgeWait) % potentially make private and remove stupid checks
            ...
            assert((strcmp(cmd,'start') && all(~stationIDsRunning(r,stationIDs))) || (strcmp(cmd,'stop') && all(stationIDsRunning(r,stationIDs))),...
                'ratrix:commandBoxIDStationIDs:incompatibleValue','to start, all stations must be stopped.  to stop, all stations must be running.')
