...
% location of RF
RFdataSource='\\132.239.158.169\datanet_storage'; % good only as long as default stations don't change, %how do we get this from the dn in the station!?
...
ts{2}= trainingStep(ap,  ffgwn, repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'searchFFGWN');       %temporal response function only slow temp freqs

% RF localization stims whiteNoise
ts{3}= trainingStep(ap,  bin6x8,       numTrialsDoneCriterion(8), noTimeOff(), svnRev, svnCheckMode,'bin6X8'); %full field gaussian white noise
ts{4}= trainingStep(ap,  bin12x16,       numTrialsDoneCriterion(8), noTimeOff(), svnRev, svnCheckMode,'bin12x16'); %full field gaussian white noise
ts{5} = trainingStep(ap,  sparseBright,  numTrialsDoneCriterion(8), noTimeOff(), svnRev, svnCheckMode,'sparseBright');  %temporal response function
ts{6} = trainingStep(ap,  sparseDark,  numTrialsDoneCriterion(8), noTimeOff(), svnRev, svnCheckMode,'sparseDark');  %temporal response function
...
% gratings chars
ts{9} = trainingStep(ap,  sfGratingsSine,  numTrialsDoneCriterion(3), noTimeOff(), svnRev, svnCheckMode,'sfGratingsSine');  %temporal response function
...
% area summation
ts{14} = trainingStep(ap,  radGratingsPPC256Ctr100,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC256Ctr100');  %temporal response function
ts{15} = trainingStep(ap,  radGratingsPPC256Ctr80,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC256Ctr80');  %temporal response function
ts{16} = trainingStep(ap,  radGratingsPPC256Ctr60,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC256Ctr60');  %temporal response function
ts{17} = trainingStep(ap,  radGratingsPPC256Ctr40,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC256Ctr40');  %temporal response function
ts{18} = trainingStep(ap,  radGratingsPPC256Ctr20,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC256Ctr20');  %temporal response function


ts{19} = trainingStep(ap,  radGratingsPPC128Ctr100,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC128Ctr100');  %temporal response function
ts{20} = trainingStep(ap,  radGratingsPPC128Ctr80,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC128Ctr80');  %temporal response function
ts{21} = trainingStep(ap,  radGratingsPPC128Ctr60,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC128Ctr60');  %temporal response function
ts{22} = trainingStep(ap,  radGratingsPPC128Ctr40,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC128Ctr40');  %temporal response function
ts{23} = trainingStep(ap,  radGratingsPPC128Ctr20,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC128Ctr20');  %temporal response function


ts{24} = trainingStep(ap,  radGratingsPPC64Ctr100,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC64Ctr100');  %temporal response function
ts{25} = trainingStep(ap,  radGratingsPPC64Ctr80,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC64Ctr80');  %temporal response function
ts{26} = trainingStep(ap,  radGratingsPPC64Ctr60,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC64Ctr60');  %temporal response function
ts{27} = trainingStep(ap,  radGratingsPPC64Ctr40,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC64Ctr40');  %temporal response function
ts{28} = trainingStep(ap,  radGratingsPPC64Ctr20,  numTrialsDoneCriterion(2), noTimeOff(), svnRev, svnCheckMode,'radGratingsPPC64Ctr20');  %temporal response function

% burnthro
ts{29} = trainingStep(ap,  gwn,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'binFast');  %temporal response function
...
p=protocol('practice phys',{ts{1:68}});
stepNum=uint8(2);
