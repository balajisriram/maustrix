function r = lgnNonDynamic(r,subjIDs,otherParams)
...
%% temporal stimuli
% full-field temporal
pixPerCycs=2.^(16); driftfrequencies=2.^(-1:4); orientations=[0]; phases=[0];
contrasts=[1]; durations=[3];
radius=5; annuli=0;
location=[.5 .5];
waveform='sine'; normalizationMethod='normalizeDiagonal';
mean=0.5; thresh=.00005; numRepeats=3;
maxWidth=1920;maxHeight=1080;scaleFactor=0;interTrialLuminance=.5;
...
% location of RF
% if IsWin
%     RFdataSource='\\132.239.159.247\onTheFly'; % good only as long as default stations don't change, %how do we get this from the dn in the station!?
% elseif IsLinux
...
% manual camera calib
numSweeps=int8(3);
background=gray;maxWidth=1920;maxHeight=1080;scaleFactor=0;interTrialLuminance=.5;
...
% ts{5} = trainingStep(ap,  findRFManual,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'findRFManual');  %temporal response function
% ts{6} = trainingStep(ap,  sfSmallFullC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'sfSmallFullC');  %temporal response function
ts{6} = trainingStep(ap,  sfFFFullC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'sfSmallFullC');  %temporal response function
% ts{8} = trainingStep(ap,  contrast512Small,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'contrast512Small');  %temporal response function
% ts{9} = trainingStep(ap,  contrast256Small,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'contrast256Small');  %temporal response function
ts{7} = trainingStep(ap,  contrast512FF,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'contrast512FF');  %temporal response function
ts{8} = trainingStep(ap,  contrast256FF,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'contrast256FF');  %temporal response function
% ts{12} = trainingStep(ap,  aSum256FullC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum256FullC');  %temporal response function
% ts{13} = trainingStep(ap,  aSum256HalfC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum256HalfC');  %temporal response function
% ts{14} = trainingStep(ap,  aSum256QuatC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum256QuatC');  %temporal response function
% ts{15} = trainingStep(ap,  aSum128FullC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum128FullC');  %temporal response function
% ts{16} = trainingStep(ap,  aSum128HalfC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum128HalfC');  %temporal response function
% ts{17} = trainingStep(ap,  aSum128QuatC,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'aSum128QuatC');  %temporal response function
% % burnthro
% ts{18} = trainingStep(ap,  burnthro,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'burnthro');  %temporal response function
% ts{19} = trainingStep(ap,  cmr,  repeatIndefinitely(), noTimeOff(), svnRev, svnCheckMode,'cmr');  %temporal response function

p=protocol('basicLGNProtocol',{ts{1:8}});
