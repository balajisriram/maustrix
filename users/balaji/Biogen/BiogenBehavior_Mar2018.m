classdef BiogenBehavior_Mar2018
    ...
    methods (Static)
        ...
        function rx = createVisionPhysiologyBCore()
            ...
            st = BiogenBehavior_Mar2018.makeVisionBehaviorStation(id,stationPath,mac,physicalLocation);
        ...
        function st=makeVisionBehaviorStation(id,path,mac,physicalLocation,~,~,~,~,~)
        ...
        function [fdAutoTrigger, fd, fdAlternate] = makeBehaviorFreeDrinkTrialManagers()
            ...
            showText = 'full';
            freeDrinkLikelihood = 0.01;

            
            fdAutoTrigger=freeDrinks(sm, rm, dm, frameDropCorner, dropFrames, requestPort, saveDetailedFrameDrops,...
                responseWindowMs, showText, freeDrinkLikelihood, true);
            fd=freeDrinks(sm, rm, dm, frameDropCorner, dropFrames, requestPort, saveDetailedFrameDrops,...
                responseWindowMs, showText, 0, true);
            fdAlternate=freeDrinksAlternate(sm, rm, dm, frameDropCorner, dropFrames, requestPort, saveDetailedFrameDrops,...
                responseWindowMs, showText, 0, true);
        end
        
        function [rm, rmWithRequest] = makeStandardReinforcementManagers(requestRatio)
            if ~exist('requestRatio','var') || isempty(requestRatio)
                requestRatio = 0.1;
            end
            ...
            rmWithRequest=constantReinforcement(rewardScalar,rewardScalar*requestRatio,penaltyScalar,puffScalar,fractionOpenTimeSoundIsOn,fractionPenaltySoundIsOn,requestMode);
        end
        
        function [tm, tmWithRequest] = makeStandardTrialManagerNAFC()
            % Create Sound Manager
            sm = BCoreUtil.makeStandardSoundManager();
            
            % Reward Managers
            requestRatio = 0.1;
            [rm, rmWithRequest] = BiogenBehavior_Mar2018.makeStandardReinforcementManagers(requestRatio);
            ...
            tmWithRequest=nAFC(sm,rmWithRequest,noDelay,frameDropCorner,dropFrames,reqPort,saveDetailedFrameDrops,customDescription,responseWindowMS,showText,percentCorrectionTrials);
        end
        ...
        function ts = freeDrinkTrainingSteps(trialManager, performanceCrit, sch, stepName)
            ...
            %afcGratings(pixPerCycs,driftfrequencies,orientations,phases,contrasts,maxDuration,radii,radiusType, annuli,location,...
