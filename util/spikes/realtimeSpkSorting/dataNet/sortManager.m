...
fOrd=sampRate/200; %how choose filter orders?
loCut=100;
hiCut=10000;
loThresh=[];
hiThresh=[];
...
f = figure('MenuBar','none','Toolbar','figure','Name','sortManager','NumberTitle','off','Resize','off','CloseRequestFcn',@cleanup,'Units','pixels','Position',[50 50 fWidth fHeight]);
...
threshLoS=uicontrol(f,'Style','slider','String','loThresh','Min',-1,'Max',0,'SliderStep',[.001 .05],...
    ...
    function drawThresh()
        ...
        plot(ah,[0 secDur],[0 0],'k');
        plot(ah,[0 secDur],ones(2,1)*[loThresh hiThresh],'k');
...
timesV=linspace(0,secDur,secDur*viewRate);
