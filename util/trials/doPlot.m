...
switch plotType
    case 'plotTrialsPerDay'
        ...
        if ismember('didHumanResponse',fields(d)) & ismember('containedForcedRewards',fields(d)) & ismember('didStochasticResponse',fields(d))
            ...
            %get counts
            humanTrialsPerDay=makeDailyRaster(d.correct,d.response,d.date,humanModified);
            computerTrialsPerDay=makeDailyRaster(d.correct,d.response,d.date,computermodified);
            CTsPerDay=makeDailyRaster(d.correct,d.response,d.date,CTs);
        ...
        set(gca,'xticklabel',datestr(xticks,'mmm-dd'))
        
        endProtocol = d.compiledLUT{d.protocolName(end)};
        endDate = datestr(d.protocolDate(end));
        xMax = max(dates);
        xMin = min(dates);
        yMax = 1000;
        
        text(xMin+(xMax-xMin)/2, yMax-(yMax/10), endProtocol, 'HorizontalAlignment', 'center');
        text(xMax, yMax-(yMax/10), endDate, 'HorizontalAlignment', 'right');
    case 'plotRewardTime'

        [d.correctInRow runEnds d.numSwitchesThisRun]=calcAmountCorrectInRow(d.correct,d.response);
        maxN=5;
        [trialsPerDay]=makeDailyRaster(d.correct,d.response,d.date);
    ...
    case 'percentCorrect'
        ...
        plotFields = fieldnames(userData);
        entries = [];
        legendStrs = {};
        for i = 1:length(plotFields)
            switch plotFields{i}
                case 'CTplot'
                    entries(end+1) = userData.CTplot;
                    legendStrs{end+1} = 'correction trials';
                case 'AEplot'
                    entries(end+1) = userData.AEplot;
                    legendStrs{end+1} = 'after errors';
                case 'Perfplot'
                    entries(end+1) = userData.Perfplot;
                    legendStrs{end+1} = 'performance';
            end
        end
        
        legend(gca,entries,legendStrs);
    ...
    case 'makeDailyRaster'
        smoothingWidth=100;
        minsPerBin=5;
        junk=makeDailyRaster(d.correct,d.response,d.date,goods,smoothingWidth,minsPerBin,subject,true,[],[],true,false); %in terms of good trials, PLOT NOT SAVED 
        
    case 'plotReinforcement'
        
        lengthData = length(d.potentialPenalty);
        yRew = 1:lengthData;
        yReqRew = 1:lengthData;
        for x = 1:lengthData
            yRew(x) = d.potentialRewardMS(x)*d.potentialRewardScalar(x);
            yReqRew = d.potentialRequestRewardMS*d.potentialRewardScalar(x);
        end
        
        [hAx,~,~] = plotyy([(1:lengthData)', (1:lengthData)'], [yRew', yReqRew'], 1:lengthData, d.potentialPenalty);
        %[hAx,~,~] = plotyy(1:lengthData, yReqRew, 1:lengthData, d.potentialPenalty);
        
        title('Changes in Reward and Penalty Length');
        xlabel('Trials');

        ylabel(hAx(1),'Reward'); % left y-axis
        ylabel(hAx(2),'Penalty'); % right y-axis
        ylim(hAx(1), [0 (max(d.potentialRewardMS)+nanstd(d.potentialRewardMS)/3)]);
        ylim(hAx(2), [0 (max(d.potentialPenalty)+nanstd(d.potentialPenalty)/3)]);
...
switch plotType
    ...
    case 'plotReinforcement'
        requiredFields={'potentialRewardMS','potentialPenalty','potentialRewardScalar'};
    otherwise
        plotType=plotType
        error('unknown plot type')
end
