...
if ~exist('whichPlots','var')
    whichPlots='plotPerformance';
end

if ~ismember('plotPerformance',whichPlots) &  ismember('plotMoreDetails',whichPlots)
    error('plotPerformance  might have to always be on for more details... check code if its off')
    whichPlots{end+1}='plotPerformance'; % always force performance on for now
    handles(end+1)=handles(end)+1;
end

possiblePlotRequests={'plotPerformance','plotTrialsPerDay','plotDPrime','plotLickAndRT',...
    'plotEvenMoreLickAndRT','plotResponseDensity','plotResponseRaster','plotRewardTime',...
    'plotMotorResponsePattern','plotFlankerAnalysis','plotTemporalROC',...
...
if all(ismember(whichPlots,possiblePlotRequests))
    
    %init to false
    for i=1:length(possiblePlotRequests)
        p.(possiblePlotRequests{i})=false;
    end
    
    %set requested to true
    for i=1:length(whichPlots)
        p.(whichPlots{i})=true;
    end
else
    whichPlots(~ismember(whichPlots,possiblePlotRequests))
    error('bad plot request')
end
...
if ~exist('handles','var')
    ...
    if size(whichPlots,2)~= size(handles,2)
        error('Handles must be defined for every Plot');
    end
end
...
if ~hasData
    ...
    for i=1:length(whichPlots)
        figure(handles(i)); subplot(subplotParams.y, subplotParams.x, subplotParams.index);
        title(subject);
...
p.plotCombinedDetails=false;  % this interacts with the saving of each plot, b/c hndles are unused, figures unmade
% if sum([p.plotDPrime p.plotLickAndRT])>0
%     if (subplotParams.x==1) && (subplotParams.y==1)
%         p.plotCombinedDetails=1;
%     else
%         warning('cant plot combined details for more than one subject.')
%         p.plotCombinedDetails=0;
%     end
% else
%     p.plotCombinedDetails=0;
% end
...
if p.plotPerformance
    %myTitle=['Performance for ' contextInfo];
    myTitle=[subject '; ' datestr(min(d.date),6) '-' datestr(max(d.date),6)];
    
    figure(handles(find(strcmp('plotPerformance',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on;
    performanceFig=gcf;
    ...
    elseif numSubplot>1 && numSubplot<=9
        ...
        myTitle=shortTitle; %maybe do something for dateRanges of all animals...
        %ideally indicate on graph if dateRange is unexpected...
        set(gca,'YTickLabel','50%|85%|100%')
    end
    
    if p.plotCombinedDetails
        subplot(311);
    end
    
    if p.plotMoreDetails
        p.plotCorrectionTrialsToo=1; %etc
    end
...
if p.plotDPrime || p.plotCombinedDetails
    if p.plotCombinedDetails
        subplot(312); hold on
    else
        figure (handles(find(strcmp('plotDPrime',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on;
    end
...
if p.plotLickAndRT || p.plotCombinedDetails
    if p.plotCombinedDetails
        subplot(313); hold on
    else
        figure (handles(find(strcmp('plotLickAndRT',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on;
    end
    
    % licks
    if ismember('numRequests',fields(d))
    plot(conv2(single(d.numRequests), ones(1,100)/100,'same'),'.','MarkerSize',1,'color',[.6,.4,.4]); hold off
    axis( [1 max([totalTrials 2]) 0 10])
    ylabel('smoothed num licks ')
    xlabel(sprintf('Trial Number (%d good of %d total)', sum(goods), length(goods)))
    else
        text(.5,.5,'no lick data')
    end
...
if p.plotMotorResponsePattern
    %PLOT REPONSE PATTERN
    figure(handles(find(strcmp('plotMotorResponsePattern',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on
    lengthHistory=3;
...
if p.plotFlankerAnalysis
    figure(handles(find(strcmp('plotFlankerAnalysis',whichPlots))));  subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on
...
if p.plotTemporalROC
    ...
    figure(handles(find(strcmp('plotTemporalROC',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index)
...
if p.plotLocallyNormalizedEffect
    ...
    if (subplotParams.x==1) && (subplotParams.y==1)
        figure(handles(find(strcmp('plotLocallyNormalizedEffect',whichPlots))));
        subplot(122); hold on
        title(who)
        ...
        figure(handles(find(strcmp('plotLocallyNormalizedEffect',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index)
        title(subject);
        effect=pct(1,:)-mean(pct(2:4,:));
        hold on;
        %errorbar(1:length(delta),delta,delta-CI(1,:),CI(2,:)-delta)
...
if p.plotContrastCurve
    
    if (subplotParams.x==1) && (subplotParams.y==1)
        figure(handles(find(strcmp('plotContrastCurve',whichPlots))));
        subplot(211);
        ...
        figure(handles(find(strcmp('plotContrastCurve',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index)
...
if p.plotBiasScatter || p.plotBias
...
if p.plotBias
    figure(handles(find(strcmp('plotBias',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index);
    plot(find(goodForBias),bias,'color',[0,0,0], 'linewidth', 2);
    axis([0 max(totalTrials,1) 0 1]);
    title(shortTitle)
end

if p.plotBiasScatter
    ...
    figure(handles(find(strcmp('plotBiasScatter',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index);
...
if p.plotRatePerDay
    ...
    figure(handles(find(strcmp('plotRatePerDay',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index); hold on
...
if p.plotPerformancePerDaysTrial
    ...
    %figure(handles(18))
    %close (handles(18))
    figure(handles(find(strcmp('plotPerformancePerDaysTrial',whichPlots)))); subplot(subplotParams.y, subplotParams.x, subplotParams.index);
    hold on
