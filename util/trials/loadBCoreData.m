function [data sessionIDs]=loadBCoreData(path,subjects,method,methodParams,dontGetData)
%loadBCoreData(path,subjects,'all',[])
%loadBCoreData(path,subjects,'sinceDate',dateNum) 
%loadBCoreData(path,subjects,'lastNFiles',numFilesToLoad)
%loadBCoreData(path,subjects,'betweenDates',[now-5 now],1)
...
switch method
    ...
    otherwise
        method
        error('that is not a known method of loading BCore data, check loadMethod')
end
